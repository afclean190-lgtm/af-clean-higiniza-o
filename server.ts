...
async function startServer() {
  ...
  // 4. API Routes (Directly on app for maximum reliability)
  app.get("/api/test", (req, res) => res.json({ status: "ok", dbConnected: !!db }));

  app.get("/api/appointments", checkDb, (req, res) => {
    try {
  ...
  app.post("/api/appointments", checkDb, (req, res) => {
    try {
  ...
  app.patch("/api/appointments/:id", checkDb, (req, res) => {
    try {
  ...
  app.delete("/api/appointments/:id", checkDb, (req, res) => {
    try {
  ...
  app.get("/api/financials", checkDb, (req, res) => {
    try {
      res.json(db.prepare("SELECT * FROM financials ORDER BY date DESC").all());
    } catch (e: any) { res.status(500).json({ error: e.message }); }
  });

  app.post("/api/financials", checkDb, (req, res) => {
    try {
  ...
  app.delete("/api/financials/:id", checkDb, (req, res) => {
    try {
